	...
	if (p_llut != NULL)	{
		...
		if (new_lut != NULL) {
			...
			if (p_llut->l_tail == NULL) {
				...
				//Pointer vers la lut précédente à NULL car il n'y a rien avant
				new_lut->l_prev = NULL; 
				//Pointer vers la lut suivante à NULL car on rajoute à la fin de la liste	
				new_lut->l_next = NULL;
...
LLut* copyLLut(LLut* p_llut, LLut* new_llut) {
		...
		if(new_llut != NULL) {
			...
			while (lut_tmp != NULL) {
				...
				if (new_lut != NULL) {
					...
					new_lut = copyLut(lut_tmp, new_lut);
				}
				else {
